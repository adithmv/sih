  ...
  return (
    <div className="gov-site">
      ...
      <div className="safety-line">
        <div className="container">🔒 {t("safety")}</div>
      </div>
      {error && (
        <div className="container error-banner" role="alert">
          <span>{error}</span>
          <button onClick={clearError} aria-label={t("dismiss")}>
            <X size={20} />
          </button>
        </div>
      )}
      ...
      <footer className="site-footer">
        <div className="container minimal-footer">
          <div>
            <strong>{t("appName")}</strong>
            <p>{t("footerLine")}</p>
          </div>
          <a href="tel:180042555214">
            <Phone size={18} /> {t("needHelp")} 1800 425 55214
          </a>
        </div>
